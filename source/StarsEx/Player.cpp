...
Player::Load()
{
    ...
    using starshatter::foundation::Count;

    const int blocklen_int = loader->LoadBuffer(filename, block, true);
    ...
    Parser parser(
        starshatter::foundation::Reader{
            reinterpret_cast<const char*>(block),
            static_cast<Count>(blocklen_int)
        }
    );
...
Player::LoadAwardTables()
{
    ...
    Parser parser(loader->Open(filename));
